...
df = pd.read_csv(os.path.join(folder, 'data/user-item-interactions.csv'))
df_content = pd.read_csv(os.path.join(folder, 'data/articles_community.csv'))
del df['Unnamed: 0']
...
def sol_2_test(top_articles):
    ...
    for idx, file in enumerate(checks):
        if set(eval(file)) == set(pickle.load(open( os.path.join(folder, "data/{}.p".format(file)), "rb" ))):
